...
- the draft-v1 truth surface now has a repo-native Rust path under `xtask` and `crates/guild-draft-truth`: `cargo run -q -p xtask -- draft-v1 truth check` is the standard validation entrypoint, `support-matrix` / `compatibility` / `benchmark` each have explicit `check` and `write` verbs, the checked support/compatibility/benchmark artifacts remain in `docs/schemas/draft-v1/` and `docs/benchmarking/` as outputs, and the old Python truth entrypoints now exist only as deprecation wrappers that forward to the Rust-native commands instead of carrying duplicate logic
