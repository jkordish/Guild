...
> - `M8c`: partial; `read-resource` has bounded live proof with checked plan -> proof -> token -> witness linkage, `log-write` has real live family proof, and `http-request` has bounded live proof only for five deterministic replay-fixtured slices over `http`: loopback IP `GET` and `HEAD`, each with an explicit-port form and an implicit-default-port form, plus explicit-port `localhost` `GET` with a deterministic loopback-only resolution binding, all with exact observed path, no query, and no redirects
> - broader `http-request` shapes, including `localhost` default-port `GET`, `localhost` `HEAD`, other hostname forms, query or fragment components, redirects, multiple exercised requests, and `https`, plus `invoke-skill` and `emit-evidence`, remain outside the live-proof envelope
