...
> - `M8c`: partial; `read-resource` has bounded live proof with checked plan -> proof -> token -> witness linkage, `log-write` has real live family proof, and `http-request` has bounded live proof only for four deterministic replay-fixtured loopback IP slices over `http`: `GET` and `HEAD`, each with an explicit-port form and an implicit-default-port form, all with exact observed path, no query, and no redirects
> - broader `http-request` shapes, including loopback hostname forms, query or fragment components, redirects, multiple exercised requests, and `https`, plus `invoke-skill` and `emit-evidence`, remain outside the live-proof envelope
