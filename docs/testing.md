...
- M5 still remains `not_proven` for other hostname forms, query or fragment components, redirects, multiple exercised `http-request` calls beyond the checked slice set, `https`, broader `invoke-skill` shapes beyond the exact single-child and exact two-child same-alias inspect slices, and broader or legacy `emit-evidence` flows beyond one exact single-emission fixed local-object-store slice. That exact `emit-evidence` slice is now proof-backed with a separate host-owned exact binding for sink and payload facts; broader flows stay fail-closed because the guest-visible canonical authority model remains intentionally coarse.
...
- supported proof-linked slices: `read-resource` immutable roots, eight bounded `http-request` replay slices, two bounded `invoke-skill` zero-authority inspect slices, and one exact single-emission `emit-evidence` fixed-sink slice
