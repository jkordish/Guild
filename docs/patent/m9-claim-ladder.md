...
- Measured scope now: `read-resource-immutable-guild-roots`, the eight measured `http-request` replay-backed slices, the two measured `invoke-skill` zero-authority slices (`invoke-skill-single-child-zero-authority` plus `invoke-skill-two-child-same-alias-zero-authority`), and `emit-evidence-single-emission-exact-local-object-store`, plus explicit fallback or refusal behavior for the measured unsupported slices and walls.
...
- Measured scope now:
  ...
  - one exact `emit-evidence` single-emission fixed local object-store slice
...
- Measured scope now:
  ...
  - exact single-sink comparator for the measured `emit-evidence` fixed local object-store slice, with broader or legacy flows still failing closed
...
- Not-claimable-yet items now:
  - broad `emit-evidence` linkage beyond the exact fixed-sink slice
