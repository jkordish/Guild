...
def build_matrix_lines(skills: list[tuple[str, dict]], runtimes: list[tuple[str, dict]]) -> list[str]:
    lines = [
        ...
        "",
        "Per-family M8b layer status now lives in `family_support_matrix.json`. This derived file is only the M4 hard-requirement precheck view.",
        "",
        ...
        "",
        "This table therefore mixes both surfaces on purpose: the new runtime-* fixtures exercise direct canonical family support, while the older bounded fixtures still prove the explicit compatibility paths that remain in scope for M8b.",
        "",
