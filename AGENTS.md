...
- the active inspect runtime now also allows only `guild:skill/inspect-types@1.0.0` and `guild:skill/inspect-host@1.0.0` at the Guild component import boundary; broader Guild imports fail closed as host-owned `unsupported-runtime-surface` rejections rather than generic runtime errors
...
- evidence metadata is now directly readable under companion `guild://objects/records/{evidence_record_id}/metadata` URIs while `guild://objects/records/{evidence_record_id}` remains the payload-dereference URI
...
- `guild` is now the real thin local CLI for init/inspect/read/install/export/import/push/pull/trust/mcp-stdio over that same substrate
- the operator-facing CLI now resolves roots as `--registry-root`, then `GUILD_REGISTRY_ROOT`, then `~/.guild`, while read-only commands fail clearly instead of silently creating a new root and proof flows stay on explicit temp or `target/dev-local-registry/...` roots
- `guild init` is now the single current local bootstrap workflow for creating the selected Guild root, printing the real `guild mcp serve --stdio` Codex wiring, and explicitly writing global or project Codex config with `--global` / `--project` against the running `guild` binary
- `guild codex` now bootstraps deterministic local dogfood roots, prints the repo-local Cargo-based stdio Codex MCP config for deterministic proofs, prepares local scenarios, and runs deterministic Codex smoke flows over that same path rather than acting as the normal operator setup surface
...
`guild init` is the current persistent Codex operator setup path: it creates the selected Guild root, prints the exact stdio launch command plus matching `codex mcp add ...` registration command and `.codex/config.toml` snippet for the running `guild` binary, and it can explicitly and idempotently update global or project Codex config files with `--global` / `--project`.
`guild codex bootstrap` remains the supported deterministic dogfood path inside this repository: it creates a fresh explicit local Guild root, installs the example skills used by the recommended Codex flows, prints the repo-local Cargo-based registration command plus matching config snippet for the real `guild mcp serve --stdio` server launch, and prints the exact follow-up `guild codex scenario ...` and `guild codex smoke ...` commands for the recommended flows.
`guild codex scenario --scenario recent-failure-triage|policy-denial-debug|execution-tree --json` prepares one deterministic local workflow root and returns JSON with subject/query URIs plus one recommended Codex ask string for that scenario.
`guild codex smoke --flow explain-execution`, `explain-execution-tree`, `recent-failure-triage`, and `policy-denial-debug` are the deterministic helper-level proofs for those same explain/debug flows over the real stdio MCP server.
