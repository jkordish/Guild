...
`guild codex smoke --flow explain-execution`, `explain-execution-tree`, `recent-failure-triage`, and `policy-denial-debug` are the deterministic helper-level proofs for those same explain/debug flows over the real stdio MCP server, while `guild-codex` remains as a compatibility alias.
