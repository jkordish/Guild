...
- `localhost` default-port `GET`, `localhost` default-port `HEAD`, other hostname forms, query or fragment components, redirects, multiple exercised requests, `https`, broader `invoke-skill` shapes including dynamic or broader resolution, multi-child fan-out, recursion, child-side authority use, and non-inspect child targets, plus `emit-evidence`, still stay explicitly `not_proven` for live proof and therefore stay on upper-bound-only or unlinked downstream behavior
