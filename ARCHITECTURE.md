...
- supported proof-linked slices only for `read-resource`, the eight bounded `http-request` replay slices, and the one bounded `invoke-skill` single-child zero-authority slice
