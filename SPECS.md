...
- supported proof-linked slices measured today are exactly one bounded `read-resource` immutable-root slice, eight bounded `http-request` replay-fixtured `http` slices, and one bounded `invoke-skill` single-child zero-authority slice
