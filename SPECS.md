...
- `http-request` MUST be described only as bounded live-proof-backed for one deterministic replay-fixtured `GET` over `http` to an explicit loopback IP-literal host plus explicit port plus exact observed path-prefix, with no query, no redirects, and the normalized inspect-output comparator only
- broader `http-request` shapes, plus `invoke-skill` and `emit-evidence`, MUST remain explicitly `not_proven` for live proof and MUST NOT be described as proof-backed for token or witness linkage
