...
- supported proof-linked slices measured today are exactly one bounded `read-resource` immutable-root slice, eight bounded `http-request` replay-fixtured `http` slices, and two bounded `invoke-skill` zero-authority inspect slices
