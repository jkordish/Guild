...
- broader `http-request` shapes, including `localhost` default-port `GET`, `localhost` default-port `HEAD`, other hostname forms, query or fragment components, redirects, multiple exercised requests, and `https`, broader `invoke-skill` shapes including dynamic or broader resolution, multi-child fan-out, recursion, child-side authority use, and non-inspect child targets, plus all `emit-evidence` flows, MUST remain explicitly `not_proven` for live proof and MUST NOT be described as proof-backed for token or witness linkage
