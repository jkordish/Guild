...
use guild_runner::{LiveProofComparatorProfile, LiveProofSupport, Runner, WasmtimeRuntimeAdapter};
use guild_types::{
...
fn http_request_live_proof_stays_not_proven_without_replay() {
    ...
    assert!(family.reason_codes.iter().any(|code| code == "LIVE_REPLAY_UNAVAILABLE"));
}
...
fn read_resource_live_proof_fails_closed_for_query_resources() {
    ...
    let family = proof_result
        ...
        .unwrap();
    assert_eq!(family.support, LiveProofSupport::NotProven);
    assert!(family.reason_codes.iter().any(|code| code == "LIVE_SCOPE_SHRINK_UNSUPPORTED"));
}
