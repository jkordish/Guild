...
pub fn render_matrix() -> Result<String> {
    let mut lines = vec![
        ...
        String::new(),
        "All bundled contracts in this directory now declare the live inspect world `guild-skill-inspect-v1`, so WIT-world checks here are aligned to the real Rust inspect entrypoint rather than the older example-local names.".to_owned(),
        String::new(),
